...
export class AuthController {
    ...
    async uploadPhoto(
        ...
        @UploadedFile(new ParseFilePipe({
            validators: [
                ...
                new MaxFileSizeValidator({message: 'Tamanho máximo de 10Mmb excedido', maxSize: 1024 * 10000})
            ],
    ...
    @UseGuards(AuthGuard)
    @Post('multiple')
    async uploadMultiple(@User() user, @UploadedFiles() files: Express.Multer.File[]) {
        return files
    }
    ...
    async uploadMultipleFields(@User() user, @UploadedFiles() files: {photo: Express.Multer.File, documents: Express.Multer.File[]}) {
        return files
    }
}